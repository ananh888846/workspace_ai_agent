...
\n\n---\n\n# Production SQL Review Lock — Migration 041 → 045 — 2026-09-21\n\nCác điểm implementation gate đã được chốt trước production SQL:\n\n- 041/042 mang organization_id và composite FK về automations(id, organization_id).\n- 043 Anomaly tenant-scoped; user/device/resource/event/activity/task references phải cùng organization. Confidence và resolved_at có CHECK.\n- 044 không dùng polymorphic source_type/source_id. Production implementation dùng bảy nullable source FK: observation_id, event_id, activity_session_id, activity_id, task_id, device_id, resource_id, cùng organization_id và CHECK đúng một source.\n- 045 Audit Log có correlation/context FK, append-only trigger cho UPDATE/DELETE và trigger chặn audit metadata chứa secret keys.\n- Audit metadata chỉ dành cho audit-safe metadata; không lưu credential/token/secret.\n\nProduction SQL 041 → 045 đã được tạo theo lock này; acceptance verification vẫn là gate bắt buộc trước khi CLOSED.\n